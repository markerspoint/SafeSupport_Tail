...
<body class="flex flex-col min-h-screen">
    <div class="relative w-full bg-center bg-cover"
        ...
        <header
            class="z-50 w-full flex items-center justify-between px-4 pt-4 pb-2 bg-white/70 backdrop-blur-md relative">
            <a href="#" class="flex items-center space-x-3">
                <img src="{{ asset('img/safesupport-logo.png') }}" alt="SafeSupport Logo"
                    class="w-[10rem] h-[3rem] object-contain">
            </a>
            ...
            <div class="flex items-center space-x-2">
                <a href="{{ route('student.messages') }}" class="relative rounded-full p-2 hidden md:flex">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        ...
                        stroke-linejoin="round" class="lucide lucide-message-circle h-6 w-6 text-green-800">
                ...
                <div x-data="{ showNotifications: false }" class="relative">
                    <button @click="showNotifications = !showNotifications"
                        class="relative rounded-full p-2 hidden md:flex">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round" class="lucide lucide-bell h-6 w-6 text-green-800">
                            <path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"></path>
                            <path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"></path>
                        </svg>
                        <span
                            class="notification-dot absolute right-1.5 top-1.5 h-2.5 w-2.5 rounded-full bg-destructive"></span>
                    </button>

                    {{-- <!-- Notification dropdown --> --}}
                    <div x-show="showNotifications" @click.away="showNotifications = false" x-transition
                        class="absolute right-0 mt-2 z-50 min-w-[8rem] overflow-hidden rounded-md border text-popover-foreground shadow-md
                        ...
                        <div class="relative overflow-hidden h-[450px]">
                            <div class="flex flex-col items-center justify-center py-8 text-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                    ...
                                    class="lucide lucide-bell mb-2 h-8 w-8 text-green-800">
                                    <path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"></path>
                                    <path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"></path>
                                </svg>
                ...
                <div x-data="{ dropdownOpen: false }" class="relative z-50 flex items-center">
                    {{-- <!-- Mobile hamburger --> --}}
                    <button @click="$dispatch('open-user-drawer')"
                        class="md:hidden flex items-center justify-center w-12 h-12 rounded-full text-green-800 hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-green-300">
                    ...
                    <button @click="dropdownOpen = !dropdownOpen"
                        class="hidden md:flex items-center py-2 pr-12 h-12 text-sm font-medium rounded-md transition-colors text-green-800">
                        <div class="w-10 h-10 overflow-hidden border border-gray-200 rounded-full">
                            <img src="{{ getUserAvatarUrl(auth()->user()) }}" alt="user"
                    ...
                    <div x-show="dropdownOpen" @click.away="dropdownOpen = false"
                        ...
                        class="hidden md:block absolute top-full right-0 mt-2 w-56 bg-white border border-gray-200 rounded-md shadow-lg z-50"
                        x-cloak>
                        <div class="py-2">
                            ...
                            <div class="px-2">
                                <a href="{{ route('student.profile') }}"
                                    class="flex items-center w-full px-3 py-2 text-sm font-semibold text-green-800 rounded-md hover:bg-green-800 hover:text-white transition-colors duration-200">
                                    <svg class="mr-2 w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none"
        ...
        {{-- bg hero --}}
        @include('partials.master-hero')

        {{-- drawer --}}
        <div x-data="{ drawerOpen: false }" @open-user-drawer.window="drawerOpen = true">
            {{-- Off-canvas drawer (mobile only) --}}
            <div x-show="drawerOpen" x-transition:enter="transition transform ease-out duration-300"
                x-transition:enter-start="translate-x-full opacity-0"
                x-transition:enter-end="translate-x-0 opacity-100"
                x-transition:leave="transition transform ease-in duration-300"
                x-transition:leave-start="translate-x-0 opacity-100"
                x-transition:leave-end="translate-x-full opacity-0" @click.away="drawerOpen = false"
                class="md:hidden fixed inset-y-0 right-0 z-[100] w-80 bg-white shadow-2xl overflow-y-auto flex flex-col justify-between"
                x-cloak>
                <div>
                    ...
                    <div class="px-6 pb-4 space-y-2">
                        <a href="{{ route('student.profile') }}" @click="drawerOpen = false"
                            class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-green-800 rounded-lg hover:bg-green-100">
                        ...
                        <a href="{{ route('student.notification') }}" @click="drawerOpen = false"
                        ...
                        <a href="{{ route('student.messages') }}" @click="drawerOpen = false"
                            class="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg 
                ...
                <div class="mt-4 mb-[3rem]">
                    ...
                    <div class="px-[1.6rem] py-4">
                        <div class="flex items-center px-6 py-2 space-x-4 bg-slate-100 rounded-lg">
                            <div
                                class="w-12 h-12 overflow-hidden border-2 border-green-200 rounded-full flex-shrink-0">
                                <img src="{{ getUserAvatarUrl(auth()->user()) }}" alt="avatar"
                                    class="object-cover w-full h-full" />
                            </div>
                    ...
                    <div class="px-6 pb-6">
                        <a href="#" @click="drawerOpen = false"
                            class="flex items-center gap-2 w-full px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-red-500">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                stroke-width="2">
                                <path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" />
                                <polyline points="16 17 21 12 16 7" />
            ...
            <template x-if="drawerOpen">
                <div @click="drawerOpen = false" class="md:hidden fixed inset-0 bg-black/75 z-50"
            ...
            {{-- Page Intro (shows above sidebar + main) --}}
            @hasSection('page_intro')
            <div class="mt-4">
                @yield('page_intro')
            </div>
            @else
            @if (View::hasSection('page_title') || View::hasSection('page_subtitle'))
                <div class="mt-4 px-4 sm:px-6 lg:px-8">
                    <div class="relative flex flex-col sm:flex-col gap-2">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            @hasSection('page_title')
                                <h1 class="text-2xl font-bold text-green-800">@yield('page_title')</h1>
                            @endif
                            @hasSection('page_actions')
                                <div class="flex items-center gap-2 sm:mt-0 mt-1">
                                    @yield('page_actions')
                                </div>
                            @endif
                        </div>
                        @hasSection('page_subtitle')
                        <p class="text-green-700 text-sm sm:text-base">@yield('page_subtitle')</p>
                        @endif
                    </div>
                </div>
            @endif
            ...
            <div x-data="{ open: true, mobileOpen: false }" class="flex flex-col md:flex-row w-full relative">
                {{-- mobile dropdown --}}
                <div class="md:hidden w-full p-4 flex flex-col items-start">
                    <button @click="mobileOpen = !mobileOpen"
                        class="flex items-center justify-between w-full rounded-lg bg-green-500 text-white py-1 px-4 font-medium shadow-md border-b-4 border-green-600 transition">
                        <div class="flex items-center gap-2">
                            @if (request()->routeIs('student.about'))
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-info h-4 w-auto"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            ...
                            @elseif (request()->routeIs('student.chat'))
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-message-circle h-5 w-5"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" viewBox="0 0 24 24">
                                    <path
                            ...
                            @elseif (request()->routeIs('student.courses'))
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-book-open h-5 w-5"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" viewBox="0 0 24 24">
                            ...
                            @elseif (request()->routeIs('student.calendar'))
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-calendar h-5 w-5"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" viewBox="0 0 24 24">
                            ...
                            @elseif (request()->routeIs('student.discussions'))
                                <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-users h-5 w-5"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" viewBox="0 0 24 24">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" :class="mobileOpen ? 'rotate-180' : ''"
                            class="lucide lucide-chevron-down h-5 w-5 transition-transform" fill="none"
                    ...
                    <div x-show="mobileOpen" x-transition
                        class="mt-2 w-full flex flex-col rounded-lg border border-green-200 overflow-hidden bg-white shadow-md">
                        <a href="#" class="flex items-center gap-2 py-1 px-4 text-green-700 hover:bg-green-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-message-square h-4 w-4"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" viewBox="0 0 24 24">
                        ...
                        <a href="#" class="flex items-center gap-2 py-1 px-4 text-green-700 hover:bg-green-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-message-circle h-4 w-4"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" viewBox="0 0 24 24">
                                <path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"></path>
                            </svg>
                        ...
                        <a href="#" class="flex items-center gap-2 py-1 px-4 text-green-700 hover:bg-green-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-book-open h-4 w-4"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" viewBox="0 0 24 24">
                        ...
                        <a href="#" class="flex items-center gap-2 py-1 px-4 text-green-700 hover:bg-green-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-calendar h-4 w-4"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" viewBox="0 0 24 24">
                                <path d="M8 2v4"></path>
                                <path d="M16 2v4"></path>
                        ...
                        <a href="{{ route('student.about') }}"
                            class="flex items-center gap-2 py-1 px-4 {{ request()->routeIs('student.about') ? 'bg-green-500 text-white border-b-4 border-green-600 shadow-lg' : 'text-green-700 hover:bg-green-100' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-info h-4 w-4" fill="none"
                ...
                {{-- sidebar desktop --}}
              <aside :class="open ? 'w-[16rem]' : 'w-[6rem]'"
                        class="hidden md:flex flex-col transition-[width] duration-300 mt-[2.5rem] pl-[2rem] overflow-hidden">
                    <div class="flex flex-col p-2 space-y-1 text-green-600">
                        <a href="#"
                            class="flex items-center rounded-lg py-3 px-2 text-sm font-medium transition-colors gap-2 hover:bg-green-100"
                            title="Discussions">
                            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-message-square h-4 w-4"
                                fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('student.about') }}"
                            class="flex items-center rounded-lg py-3 px-2 text-sm font-medium gap-2 transition-all
                    ...
                    <div class="flex justify-center p-2">
                        <button @click="open = !open"
                            class="inline-flex items-center justify-center h-8 w-8 p-0 rounded-md text-green-700 hover:text-green-900 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" x-show="!open" class="h-4 w-4" fill="none"
                                stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                stroke-linejoin="round" viewBox="0 0 24 24">
                ...
                {{-- body --}}
                <main class="relative flex-1 z-10 p-4 sm:p-6 md:p-8 lg:p-12 overflow-hidden">
                    @yield('body')
                </main>
            </div>
        </div>
