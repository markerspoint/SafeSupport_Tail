...
<div class="max-w-7xl mx-auto p-4">
    <div class="bg-white border border-gray-200 rounded-2xl p-5 custom-shadow transition">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-gray-700 font-semibold text-lg">Self-Help Resources</h2>
            <button 
                @click="showForm = !showForm" 
                class="bg-emerald-500 text-white px-4 py-1 text-sm font-medium rounded-full transition hover:bg-emerald-600"
            >
                <span x-text="showForm ? 'Cancel' : 'Add Self-Help'"></span>
            </button>
        </div>

        <!-- Form for adding self-help resources -->
        <div x-data="{ showForm: false }" x-show="showForm" class="mb-6">
            <form action="{{ route('counselor.resources.self-help.store') }}" method="POST">
                @csrf
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($selfHelps as $selfHelp)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-2 text-gray-700">{{ $selfHelp->title }}</td>
                            <td class="px-4 py-2 text-gray-700">{{ $selfHelp->description ?? 'N/A' }}</td>
                            <td class="px-4 py-2">
                                <a href="{{ $selfHelp->url }}" class="text-emerald-500 hover:underline" target="_blank">View</a>
                            </td>
                            <td class="px-4 py-2">
                                <span class="inline-block px-3 py-1 text-xs font-semibold rounded-full shadow-sm {{ $selfHelp->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-700' }}">
                                    {{ $selfHelp->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                        </tr>
                    @empty
